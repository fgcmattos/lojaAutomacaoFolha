Public Class FrmFolhaIR_TabelaCadastro
    ...
    Private Sub BtnGravaTabela_Click(sender As Object, e As EventArgs) Handles BtnGravaTabela.Click
        ...
        If IsNumeroDeFaixas > 5 Then
            With oi

                .Msg = "Número de faixas maior do que 5 " & Chr(13) & "Sistema não está preparado, entre em contato com o administrador"
                .Style = vbExclamation
                MsgBox(.Msg, .Style, .Title)
                Exit Sub
        ...
        Query = "Insert into folha_tabela_imposto_renda_pf "
        Query += "("
        If IsNumeroDeFaixas >= 1 Then
            Query += "IRfaixa1"
            Query += ",IRfaixa1Porcentagem"
            Query += ",IRfaixa1Valor"
            Query += ",IRfaixa1Acumulado"
        End If
        ...
        If IsNumeroDeFaixas >= 2 Then
            ...
            Query += ",IRfaixa2Valor"
            Query += ",IRfaixa2Acumulado"
        End If
        If IsNumeroDeFaixas >= 3 Then
            Query += ",IRfaixa3"
            Query += ",IRfaixa3Porcentagem"
            Query += ",IRfaixa3Valor"
            Query += ",IRfaixa3Acumulado"
        End If
        If IsNumeroDeFaixas >= 4 Then
            Query += ",IRfaixa4"
            Query += ",IRfaixa4Porcentagem"
            Query += ",IRfaixa4Valor"
            Query += ",IRfaixa4Acumulado"
        End If

        If IsNumeroDeFaixas >= 5 Then
            Query += ",IRfaixa5"
            Query += ",IRfaixa5Porcentagem"
            Query += ",IRfaixa5Valor"
            Query += ",IRfaixa5Acumulado"
        End If
        ...
        Query += ",IR_ativa"
        Query += ",IRref"
        Query += ",IRresponsavelDigitacao"
        Query += ",IRresponsavelDigitacaoTipo"
    ...
    Private Function TabeleIRchecagem() As Boolean
    ...
    Private Sub FrmFolhaIR_TabelaCadastro_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        oi.Title = Me.Text


        Dim Query As String = "Select count(*) from folha_tabela_imposto_renda_pf where IRtabelaStatus = 0"
        ...
        Query = "Select count(*) from folha_tabela_imposto_renda_pf where IRtabelaStatus = 1"
        ...
        Query = "Select count(*) from folha_tabela_imposto_renda_pf where IRtabelaStatus = 2"
